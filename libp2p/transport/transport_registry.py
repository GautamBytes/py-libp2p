...
class TransportRegistry:
    ...
    def _register_default_transports(self) -> None:
        ...
        # Register WebSocket transport for /ws protocol (lazy import to handle missing deps)
        try:
            from libp2p.transport.websocket.transport import WebsocketTransport
            self.register_transport("ws", WebsocketTransport)
        except ImportError:
            logger.warning("WebSocket transport not available (trio-websocket not installed)")
    ...
    def create_transport(
        self, protocol: str, upgrader: Optional[TransportUpgrader] = None, **kwargs: Any
    ) -> Optional[ITransport]:
        """
...
def create_transport_for_multiaddr(
    ...
    try:
        ...
        logger.warning(
            ...
            f"multiaddr: {maddr}. Supported protocols: {_global_registry.get_supported_protocols()}"
        )
        return None
