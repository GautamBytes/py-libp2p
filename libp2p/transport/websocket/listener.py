...
try:
    ...
    logger.error("trio_websocket is required for WebSocket transport. Install with: pip install trio-websocket")
...
class WebsocketListener(IListener):
    ...
    def __init__(self, handler: THandler, upgrader: TransportUpgrader) -> None:
        ...
        self._listeners: Optional[Any] = None
    ...
    async def listen(self, maddr: Multiaddr, nursery: trio.Nursery) -> bool:
        ...
        if not addr_str.endswith("/ws"):
            raise ValueError(f"WebsocketListener only supports /ws addresses, got {maddr}")
        ...
        try:
            ...
            logger.debug("WebsocketListener.listen returning True with WebSocketServer object")
            return True
    ...
    def get_addrs(self) -> tuple[Multiaddr, ...]:
        ...
        try:
            ...
            if hasattr(self._listeners, "port"):
                ...
                return tuple(
                    self._multiaddr_from_socket(listener.socket) for listener in self._listeners
                )
            else:
    ...
    def _multiaddr_from_socket(self, sock: Any) -> Multiaddr:
        ...
        try:
            ...
            if len(sockname) >= 2:
                ...
                # Fallback
                return Multiaddr(f"/ip4/127.0.0.1/tcp/8080/ws")
        except Exception as e:
