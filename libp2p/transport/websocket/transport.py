...
class WebsocketTransport(ITransport):
    ...
    async def dial(self, maddr: Multiaddr) -> RawConnection:
        ...
        if not addr_str.endswith("/ws"):
            raise ValueError(f"WebsocketTransport only supports /ws addresses, got {maddr}")
    ...
    def _build_websocket_url(self, host: str, port: int) -> str:
        ...
        if self._is_ipv6_address(host) and not (host.startswith('[') and host.endswith(']')):
    ...
    def create_listener(self, handler: THandler) -> IListener:
