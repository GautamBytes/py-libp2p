...
def test_imports():
    ...
    try:
        ...
        if ws_available:
            ...
            print("⚠️  WebSocket transport not registered (trio-websocket not installed)")
...
def test_ipv6_formatting():
    ...
    try:
        ...
        def _build_websocket_url(host, port):
            if _is_ipv6_address(host) and not (host.startswith('[') and host.endswith(']')):
    ...
    try:
        ...
        for protocols, should_pass, desc in test_cases:
            ...
            if result == should_pass:
                ...
                print(f"❌ {desc:<25} {protocols} (expected: {should_pass}, got: {result})")
                all_passed = False
...
def test_transport_selection():
    ...
    try:
        from libp2p.transport.transport_registry import create_transport_for_multiaddr, TransportRegistry
        ...
        for protocols, expected_type, desc in test_cases:
            ...
            if expected_type is None:
                if transport is None:
                    ...
                    print(f"❌ {desc:<30} -> {type(transport).__name__} (expected None)")
                    all_passed = False
            else:
                if transport is not None:
                    ...
                    if expected_type.lower() in transport_name.lower():
                        ...
                        print(f"❌ {desc:<30} -> {transport_name} (expected {expected_type})")
                        all_passed = False
                else:
